...
function getColsForRender(
  ...
  | any
  | { _newColSkeleton: true }
  | { _swapColSkeleton: true; slotHeight?: number }
> {
  if (!preview || !preview.valid || preview.targetGridId !== gridId)
    return cols;
  if (preview.isColumnSwap && fromColId) {
    ...
    const r: Array<any | { _swapColSkeleton: true; slotHeight?: number }> = [
      ...cols,
    ];
    [r[si], r[ti]] = [r[ti], r[si]];
...
function getTilesForRender(
  ...
  { tile: any; origIndex: number } | { _slot: true; slotHeight?: number }
> {
  ...
  if (
    !preview ||
    preview.newColumn ||
    preview.isColumnSwap ||
    !preview.valid ||
    preview.targetColId !== col.ColId ||
    preview.targetGridId !== gridId
  ) {
    return withIdx;
  }
  const result: Array<
    { tile: any; origIndex: number } | { _slot: true; slotHeight?: number }
  > = [...withIdx];
  result.splice(preview.insertIndex, 0, {
    _slot: true as const,
    slotHeight: preview.slotHeight,
  });
  return result;
}
    ...
    <>
      {tileGrids.map((grid: any, gridIdx: number) => {
        ...
        const canAddColumn =
          ...
          !(
            cols.length === 2 &&
            cols.some((c: any) => (c.Tiles ?? []).length > 1)
          );

        const renderedCols = getColsForRender(
          cols,
          grid.InfoId,
          tileDropPreview,
          tileDragFromColId,
        ...
        if (
          ...
          const _targetCol = cols.find(
            (c: any) => c.ColId === tileDropPreview.targetColId,
          );
          const _oppCol = cols.find(
            (c: any) => c.ColId !== tileDropPreview.targetColId,
          );
          if (_targetCol && _oppCol && (_oppCol.Tiles ?? []).length === 1) {
            ...
            previewLongHeight =
              newCount * TILE_H + Math.max(0, newCount - 1) * TILE_GAP;
          }
        }
        ...
        const dropZoneId =
          overrideAddBtnInsertBeforeInfoId !== undefined
            ? overrideAddBtnInsertBeforeInfoId
            : gridIdx < tileGrids.length - 1
              ? tileGrids[gridIdx + 1].InfoId
              : null;
        const isAddRowDropActive =
          isDraggingTile &&
              ...
              {renderedCols.map((colOrSkel: any) => {
                if (colOrSkel._newColSkeleton) {
                  return (
                    <div
                      key="new-col-skel"
                      className="phone-column phone-column--new-col-preview"
                    />
                  );
                }
                if (colOrSkel._swapColSkeleton) {
                  return (
                    <div key="swap-col-skel" className="phone-column">
                      <div
                        className="block-drop-zone block-drop-zone--in-col"
                        style={{ height: colOrSkel.slotHeight ?? TILE_H }}
                      />
                    </div>
                  );
                }
                ...
                const tilesForRender = getTilesForRender(
                  col,
                  grid.InfoId,
                  tileDropPreview,
                );
                ...
                return (
                  <div
                    ...
                    ref={(el) =>
                      onColRef?.(col.ColId, el as HTMLElement | null)
                    }
                  >
                    {tilesForRender.map((item: any) => {
                      ...
                      const bg = tile.BGImageUrl
                        ? undefined
                        : resolveColor(tile.BGColor, themeColors);
                      const isSelected =
                        interactive && selectedTileId === tile.Id;
                      const isAnalysisHighlight =
                        analysisHighlightTileId === tile.Id;
                      ...
                      const canEdit = isSelected && !isDraggingThis && !!onEditTile;
                      ...
                      return (
                        <div
                          ...
                          className={[
                            ...
                            isAnalysisHighlight
                              ...
                              : "",
                            activeNavTileIds?.has(tile.Id)
                              ? "phone-tile-wrap--nav-active"
                              : "",
                              ...
                              ? () => {
                                  ...
                                  if (
                                    (tile.Action?.ObjectType ===
                                      ...
                                      tile.Action?.ObjectType ===
                                        "BulletinBoard" ||
                                      tile.Action?.ObjectType === "Calendar" ||
                                      tile.Action?.ObjectType ===
                                        "MyActivity" ||
                                      tile.Action?.ObjectType === "Map") &&
                        ...
                        >
                          ...
                          >
                            {tile.BGImageUrl && (
                              <>
                                ...
                                <div
                                  ...
                                  style={{
                                    background: `rgba(0,0,0,${(Number(tile.Opacity ?? 0) / 100).toFixed(2)})`,
                                  }}
                                />
                              </>
                            )}
                            {hasIcon && (
                              <div
                                className={`phone-tile-element${showDelIcon ? " phone-tile-element--deletable" : ""}`}
                              >
                                <span
                    ...
                    {isFreeResizeOppCol &&
                      (freeResizePreview?.extraSkeletonCount ?? 0) > 0 &&
                      Array.from({
                        length: freeResizePreview!.extraSkeletonCount,
                      }).map((_, i) => (
                        <div
                          key={`fr-skeleton-${i}`}
                          className="block-drop-zone block-drop-zone--in-col"
