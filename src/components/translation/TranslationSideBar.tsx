...
export function TranslationSideBar({
  ...
  useEffect(() => {
    ...
    getTranslatedPage(activePageId, selectedLang)
      .then((result) => { if (!cancelled) applyResult(result); })
  ...
  function handleTileBlur(bi: number, ci: number, ti: number, value: string) {
    ...
    const next = { ...sdtPage, PageStructure: { ...sdtPage.PageStructure, InfoContent: content } };
    setSdtPage(next);
    saveNow(next);
    setEditingKey(null);
  }
  ...
  function renderEditableBlocks() {
    ...
    while (i < displayContent.length) {
      ...
      if (block.InfoType === "TileGrid") {
                  ...
                  return (
                    <div key={tile.Id} className="phone-tile-wrap">
                      ...
                      >
                        ...
                        <div className={`phone-tile-element${tile.Align === "left" ? " phone-tile-element--left" : ""}`}>
                          {editingKey === tileKey ? (
                            <input
        ...
        out.push(<DescriptionBlock key={block.InfoId} block={block} interactive={false} />);
        i++;
      } else if (block.InfoType === "Images") {
        out.push(<ImageBlock key={block.InfoId} block={block} interactive={false} />);
        i++;
      } else if (
        block.InfoType === "Cta" &&
