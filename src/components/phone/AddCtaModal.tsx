...
function validatePhone(v: string): string | null {
  return /^[+]?[\d\s\-(). ]{6,20}$/.test(v.trim()) ? null : i18n.t("cta_modal_forms.phone_error");
}
function validateEmail(v: string): string | null {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.trim()) ? null : i18n.t("cta_modal_forms.email_error_addr");
}
function validateUrl(v: string): string | null {
  return /^https?:\/\/.+/.test(v.trim()) ? null : i18n.t("cta_modal_forms.url_error");
}

function prefill(ctaType: string, supplier: any, defaultLabels: Record<string, string>): { label: string; action: string } {
  const companyName = supplier.SupplierGenCompanyName || supplier.SupplierGenContactName || '';
  switch (ctaType) {
    case 'Phone':   return { label: defaultLabels.Phone,   action: supplier.SupplierGenContactPhone ?? '' };
    case 'Email':   return { label: defaultLabels.Email,   action: supplier.SupplierGenEmail ?? '' };
    case 'Weblink': return { label: companyName || defaultLabels.Weblink, action: supplier.SupplierGenWebsite ?? '' };
    case 'Address': {
      const parts = [
    ...
    default: return { label: defaultLabels[ctaType] ?? 'Button', action: '' };
  }
}
...
export function AddCtaModal({ ctaType, onConfirm, onCancel, hideSupplier = false }: AddCtaModalProps) {
  ...
  const config: Record<string, { title: string; actionLabel: string; actionPlaceholder: string; validate: (v: string) => string | null }> = {
    Phone:   { title: i18n.t("cta_modal_forms.phone.modal_title"),    actionLabel: i18n.t("cta_modal_forms.phone.field_label"),    actionPlaceholder: '+31 6 1234 5678',       validate: validatePhone },
    Email:   { title: i18n.t("cta_modal_forms.email.modal_title"),    actionLabel: i18n.t("cta_modal_forms.email.field_label"),    actionPlaceholder: 'you@example.com',       validate: validateEmail },
    Weblink: { title: i18n.t("cta_modal_forms.web_link.modal_title"), actionLabel: i18n.t("cta_modal_forms.web_link.url_label"),   actionPlaceholder: 'https://example.com',   validate: validateUrl },
    Address: { title: i18n.t("cta_modal_forms.address.modal_title"),  actionLabel: i18n.t("cta_modal_forms.address.field_label"),  actionPlaceholder: 'Street, City, Country', validate: v => v.trim() ? null : i18n.t("cta_modal_forms.address_required") },
    Form:    { title: i18n.t("cta_modal_forms.form.modal_title"),     actionLabel: i18n.t("cta_modal_forms.form.field_label"),     actionPlaceholder: '',                      validate: v => v.trim() ? null : i18n.t("cta_modal_forms.form_required") },
  };
    ...
    <div className="acm-overlay" onMouseDown={handleOverlayMouseDown}>
      <div className="acm-modal">
        ...
        <div className="acm-body">
          {!hideSupplier && (
            <>
              <label className="acm-label">
                {i18n.t("cta_modal_forms.connect_supplier_label")} <span className="acm-optional">({i18n.t("optional")})</span>
              </label>
              <div className="acm-select-wrap">
                ...
                >
                  <option value="">{i18n.t("cta_modal_forms.select_supplier_placeholder")}</option>
          ...
          {isForm ? (
            <div className="acm-select-wrap">
              ...
              >
                <option value="">{i18n.t("cta_modal_forms.select_form_placeholder")}</option>
                {forms.map(f => (
        ...
        <div className="acm-footer">
          <button className="acm-btn acm-btn--primary" type="button" onClick={handleSave} disabled={!canSave}>{i18n.t("cta_modal_forms.save")}</button>
          <button className="acm-btn acm-btn--secondary" type="button" onClick={onCancel}>{i18n.t("cta_modal_forms.cancel")}</button>
        </div>
      </div>
    </div>,
