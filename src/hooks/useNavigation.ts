...
export function useNavigation() {
  ...
  function handleTileNavigate(pageId: string, parentIndex: number) {
    ...
    setNavStack(prev =>
      prev[insertAt] === pageId ? prev.slice(0, insertAt + 1) : [...prev.slice(0, insertAt), pageId]
    );
    setNavContents(prev => {
      if (prev[pageId] !== undefined) return prev;
      const page = (dataStore.get('Current_Version')?.Page ?? []).find((p: any) => p.PageId === pageId);
      if (!page?.PageStructure) return { ...prev, [pageId]: [] };
      try {
        return { ...prev, [pageId]: JSON.parse(page.PageStructure).InfoContent ?? [] };
      } catch {
        return { ...prev, [pageId]: [] };
      }
    });
  ...
  function handleCollapseDescendants(parentIndex: number) {
    setNavStack(prev => prev.length <= parentIndex + 1 ? prev : prev.slice(0, parentIndex + 1));
  }
  ...
  function handleDeletePage(pageId: string) {
    ...
    if (!cv) return;
    dataStore.set('Current_Version', { ...cv, Page: (cv.Page ?? []).filter((p: any) => p.PageId !== pageId) });
