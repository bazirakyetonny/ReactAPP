...
export function getAppVersions(): Promise<SDTAppVersion[]> {
  return apiGet<{ AppVersions: SDTAppVersion[] }>('/api/toolbox/v2/appversions').then(
    (d) => d.AppVersions ?? []
  );
}

export function getActiveAppVersion(): Promise<SDTAppVersion> {
  return apiGet<{ AppVersion: SDTAppVersion }>('/api/toolbox/v2/appversion').then(
    (d) => d.AppVersion
  );
}
...
export function createAppVersion(payload: CreateAppVersionPayload): Promise<SDTAppVersion> {
  return apiPost<{ AppVersion: SDTAppVersion }>('/api/toolbox/v2/create-appversion', {
    ...
    VersionTemplatePagesCollection: payload.VersionTemplatePagesCollection ?? [],
    TemplateCategoryId: payload.TemplateCategoryId ?? NULL_UUID,
  }).then((d) => d.AppVersion);
}

export function copyAppVersion(appVersionId: string, appVersionName: string): Promise<SDTAppVersion> {
  return apiPost<{ AppVersion: SDTAppVersion }>('/api/toolbox/v2/copy-appversion', {
    AppVersionId: appVersionId,
...
export function copyHistoryVersion(
  ...
  return apiPost<{ AppVersion: SDTAppVersion }>('/api/toolbox/v2/copy-history-version', {
    AppVersionId: appVersionId,
...
export function restoreHistoryVersion(appVersionId: string, historyNumber: number): Promise<void> {
...
export function updateAppVersion(
  ...
  return apiPost<{ AppVersion: SDTAppVersion }>('/api/toolbox/v2/update-appversion', {
    AppVersionId: appVersionId,
    AppVersionName: appVersionName,
...
export function activateAppVersion(appVersionId: string): Promise<SDTAppVersion> {
  return apiPost<{ AppVersion: SDTAppVersion }>('/api/toolbox/v2/activate-appversion', {
    AppVersionId: appVersionId,
  }).then((d) => d.AppVersion);
}
...
export function updateVersionTranslationLanguages(
  ...
  return apiPost<unknown>('/api/toolbox/v2/update-version-translation-languages', {
    AppVersionId: appVersionId,
    TranslateLanguages: JSON.stringify(languages),
  }).then(() => undefined);
}

export function publishAppVersion(appVersionId: string, notify = false): Promise<void> {
